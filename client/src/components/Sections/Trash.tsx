...
const Trash = ({notes , setSelectedNote , selectedNote}:{notes:Note[] , setSelectedNote:Dispatch<Note|undefined> , selectedNote:undefined|Note}) => {
    return (
        <>
            {notes && notes.length > 0 ? notes.map(note => (
                    note.deleted && (
                        <Container key={note._id} onClick={() => setSelectedNote(note)}
                            ...
                            <Pin size={28} strokeWidth={0.75} color='white'/>
                            <div className={allNotes.noteText}>
                                {note.title.length < 1 || note.body.length < 1 ?
